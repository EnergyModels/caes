...
savename = "Fig6b_uncertainty_parameters.png"

# figure resolution
DPI = 400  # Set resolution for saving figures

x_vars = ["n_cmp1", "loss_m_air", "depth_m", "thickness_m", "permeability_mD", "porosity"]
x_labels = ["Polytropic index\n(-)", "Air leakage\n(%)", "Depth\n(m)", "Thickness\n(m)", "Permeability\n(mD)",
            "Porosity\n(-)"]

x_converts = [1.0, 100.0, 1.0, 1.0, 1.0, 1.0, ]
x_limits = [[], [], [0.0, 5500.0], [], [], []]
x_scales = ["linear", "linear", "linear", "log", "log", "linear"]
...
# Create plot
f, a = plt.subplots(2, 3, sharey=True, squeeze=False)
a = a.ravel()
...
a[4].legend(handles=patches, bbox_to_anchor=(0.5, -0.2), loc="upper center", ncol=3)
