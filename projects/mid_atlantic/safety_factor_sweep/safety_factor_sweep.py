...
def parameter_sweep(sweep_input, debug=True):
    ...
    while abs(kW_out_actual - kW_out) / kW_out + abs(kWh_out_actual - kWh_out) / kWh_out > error:
        ...
        inputs['safety_factor'] = sweep_input['safety_factor']  # formation permeability [mD]
...
if __name__ == '__main__':
    ...
    capacities = np.array([200])  # [MW]
    durations = np.array([24])  # [hr] (168 = 1 week)

    safety_factors = np.arange(0.1, 0.51, 0.05)
    ...
    entries = ['depth_m', 'thickness_m', 'porosity', 'capacity_MW', 'duration_hr', 'permeability_mD', 'k_type', 'safety_factor']
